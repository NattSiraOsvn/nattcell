...
import { persona_id as PersonaID, chat_message as ChatMessage } from '../types';
import { generate_persona_response } from '../services/gemini-service';
import AIAvatar from './AIAvatar';

const ChatConsultant: React.FC = () => {
  const [input, setInput] = useState('');
  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: '1', role: 'model', content: 'Chào Anh Natt, Thiên sẵn sàng hỗ trợ Anh rà soát Shard dữ liệu.', persona_id: PersonaID.thien, timestamp: Date.now(), type: 'text' }
  ]);
  const [isThinking, setIsThinking] = useState(false);

  const handleSend = async () => {
    if (!input.trim()) return;
    const userMsg: ChatMessage = { id: Date.now().toString(), role: 'user', content: input, persona_id: PersonaID.thien, timestamp: Date.now(), type: 'text' };
    ...
    try {
      ...
      const aiMsg: ChatMessage = { id: (Date.now()+1).toString(), role: 'model', content: res.text, persona_id: PersonaID.thien, timestamp: Date.now(), type: 'text' };
      ...
      <div className="flex-1 overflow-y-auto space-y-6 no-scrollbar pb-10">
        {messages.map(m => (
          <div key={m.id} className={`flex gap-4 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
             <AIAvatar personaId={m.persona_id} size="sm" />
