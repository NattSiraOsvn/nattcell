...
import { 
  ...
  Clock, Play, Pause, RotateCcw, AlertCircle, Eye
} from 'lucide-react';
...
const MonitoringDashboard: React.FC = () => {
  ...
  // Simple Mini Chart Component (CSS Bars)
    ...
    <div className="flex items-end gap-0.5 h-8 w-full mt-2">
       {Array.from({ length: 15 }).map((_, i) => (
          <div 
             ...
             style={{ height: `${20 + Math.random() * 80}%`, opacity: 0.6 + (i/20) }}
          />
       ))}
    </div>
  );
    ...
    <div className="h-full flex flex-col bg-[#020202] p-8 lg:p-12 overflow-y-auto no-scrollbar gap-10 animate-in fade-in duration-1000 pb-40 relative">
      ...
      <main className="grid grid-cols-1 xl:grid-cols-12 gap-10">
         ...
         <div className="xl:col-span-8 space-y-8">
            <div className="ai-panel p-10 bg-black/40 border-white/5 flex flex-col shadow-2xl relative overflow-visible">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
                  {healthStates.map(cell => {
                    ...
                    return (
                      <div 
                        key={cell.cell_id} 
                        onClick={() => setActiveCellId(isActive ? null : cell.cell_id)}
                        className={`p-6 rounded-[2.5rem] border transition-all duration-300 relative cursor-pointer ${
                            isActive ? 'bg-black border-cyan-500 shadow-[0_0_40px_rgba(6,182,212,0.2)] z-20 scale-105' : 
                            cell.status === 'HEALTHY' ? 'bg-white/[0.02] border-white/5 hover:bg-white/[0.05]' : 'bg-amber-500/5 border-amber-500/30'
                        }`}
                      >
                         <div className="flex justify-between items-start mb-6">
                            <div>
                               <h4 className="text-lg font-bold text-white uppercase tracking-tighter flex items-center gap-2">
                                  {cell.cell_id}
                                  {isActive && <Eye size={12} className="text-cyan-400 animate-pulse" />}
                               </h4>
                         ...
                         {/* Default View */}
                         <div className={`transition-opacity duration-300 ${isActive ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'}`}>
                         ...
                         {/* Expanded Lens View */}
                         {isActive && (
                            <div className="mt-2 animate-in fade-in zoom-in-95 duration-300 space-y-4">
                               <div>
                                  <div className="flex justify-between text-[8px] font-black text-gray-500 uppercase tracking-widest mb-1">
                                     <span>CPU Load</span>
                                     <span className="text-cyan-400 font-mono">32%</span>
                                  </div>
                                  <MiniSparkline color="bg-cyan-500" />
                               </div>
                               <div>
                                  <div className="flex justify-between text-[8px] font-black text-gray-500 uppercase tracking-widest mb-1">
                                     <span>Memory</span>
                                     <span className="text-purple-400 font-mono">128MB</span>
                                  </div>
                                  <MiniSparkline color="bg-purple-500" />
                               </div>
                               <div className="p-3 bg-white/5 rounded-xl border border-white/10 mt-4">
                                  <p className="text-[8px] font-black text-gray-500 uppercase mb-2">Recent Logs (Last 3)</p>
                                  <div className="space-y-1 font-mono text-[9px] text-gray-400">
                                     <p>> Initializing handshake...</p>
                                     <p>> Syncing warehouse_db...</p>
                                     <p className="text-green-500">> Node Active. Latency 12ms.</p>
                                  </div>
                               </div>
                               {cell.message && (
                                  <p className="text-[9px] text-amber-400 italic">"Alert: {cell.message}"</p>
                               )}
                            </div>
                         )}
                      </div>
               ...
               <div className="space-y-4">
                  {coordinationTasks.map(task => (
                    <div key={task.id} className="p-5 bg-white/[0.01] border border-white/5 rounded-3xl flex justify-between items-center hover:bg-white/[0.03] transition-all">
                       ...
                       <button className="px-5 py-2 bg-white/5 border border-white/10 rounded-xl text-[9px] font-black uppercase text-gray-500 cursor-not-allowed">Locked</button>
                    </div>
                  ))}
               </div>
            </div>
         ...
         <div className="xl:col-span-4 space-y-8">
            ...
            <div className="ai-panel p-8 bg-red-950/5 border-red-500/20 flex flex-col gap-6 relative overflow-hidden">
               ...
               <div className="pt-6 border-t border-white/5">
                  <button className="w-full py-4 bg-red-600/10 border border-red-500/20 text-red-500 rounded-2xl text-[9px] font-black uppercase cursor-not-allowed">
                     ACTIVATE HARD LOCKDOWN (TEST)
                  </button>
               </div>
            </div>
         </div>
