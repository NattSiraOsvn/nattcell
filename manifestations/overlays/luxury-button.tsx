...
import { Natt3DIcon, IconType } from '../../components/common/Natt3DIcon.tsx';
...
export const LuxuryButton: React.FC<LuxuryButtonProps> = ({
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute inset-0 bg-white/20 opacity-0 group-hover:opacity-30 transition-opacity"
        animate={(loading || isProcessing) ? { opacity: [0.2, 0.4, 0.2] } : {}}
      ...
      <div className="relative flex items-center gap-2 z-11">
        <AnimatePresence mode="wait">
          {(loading || isProcessing) ? (
            ...
            >
              <Natt3DIcon type="radar" size={currentSize.iconSize} pulse />
            </motion.div>
          ) : icon ? (
            <motion.div key="icon" initial={{ scale: 0 }} animate={{ scale: 1 }}>
              <Natt3DIcon type={icon as any} size={currentSize.iconSize} />
            </motion.div>
          ) : null}
        </AnimatePresence>
