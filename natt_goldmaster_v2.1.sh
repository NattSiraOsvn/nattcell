#!/bin/bash
# ================================================================
# NATT-OS GOLD MASTER OPERATIONS v2.1
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║     NATT-OS GOLD MASTER OPERATIONS v2.1                      ║${NC}"
...
$AUDIT_ONLY && echo -e "${BOLD}${CYAN}║     MODE: AUDIT-ONLY (chạy audit, không ảnh hưởng lock/registry) ║${NC}"
echo -e "${BOLD}╚══════════════════════════════════════════════════════════════╝${NC}"
...
TSC_ERRORS=$(npx tsc --noEmit 2>&1 | grep -c 'error TS' | head -1 || echo "0")
log_info "TSC errors: $TSC_ERRORS"

# ── [F2] Detect tất cả alias từ tsconfig ─────────────────────────
ALIAS_PREFIXES=()
if [ -f "tsconfig.json" ]; then
  while IFS= read -r line; do
    ...
    for (const k of Object.keys(paths)) {
      if (String(paths[k][0]).includes('src')) {
        console.log(k.replace('/*',''));
      }
    }
  " 2>/dev/null || true)
fi
if [ ${#ALIAS_PREFIXES[@]} -eq 0 ]; then
  log_info "Không tìm thấy alias, dùng đường dẫn tương đối."
else
  log_info "Alias detected: ${ALIAS_PREFIXES[*]}"
fi

# ── Helper: grep import theo nhiều alias ────────────────────────
import_grep() {
  local TARGET_DIR="$1"
  local PATTERN="$2"
  local COUNT=0
  # Đường dẫn thường
  cnt=$(grep -r "$PATTERN" "$TARGET_DIR" --include="*.ts" --include="*.tsx" 2>/dev/null \
        | grep -E "import|from|require" | grep -v "//\|test\|spec" | wc -l | tr -d ' ' || echo 0)
  COUNT=$((COUNT + cnt))
  # Các alias
  for alias in "${ALIAS_PREFIXES[@]}"; do
    cnt=$(grep -r "${alias}/${PATTERN}" "$TARGET_DIR" --include="*.ts" --include="*.tsx" 2>/dev/null \
          | grep -E "import|from|require" | grep -v "//\|test\|spec" | wc -l | tr -d ' ' || echo 0)
    COUNT=$((COUNT + cnt))
  done
  echo "$COUNT"
...
if ! $AUDIT_ONLY; then
  ...
  if command -v git &>/dev/null; then
    ...
    if [ "$DIRTY" -gt 0 ]; then
      ...
      COMMIT_MSG="[BĂNG SNAPSHOT v2.1] $(date '+%Y%m%d_%H%M%S') | tsc=$TSC_ERRORS"
...
function walkLayer(dirPath){
  ...
  function walk(d){
    fs.readdirSync(d).forEach(f=>{
      ...
      if(stat.isDirectory()) {
        if (f !== 'node_modules' && !f.startsWith('__tests__')) walk(fp);
      } else {
        if ( (f.endsWith('.ts') || f.endsWith('.tsx') || f.endsWith('.json')) &&
             !f.endsWith('.spec.ts') && !f.endsWith('.test.ts') && !f.includes('__tests__') ) {
...
function buildLock(baseDir,key,version,note){
  ...
  return {
    meta:{version,generated_at:new Date().toISOString(),generated_by:'bang-goldmaster-v2.1',
...
const il=buildLock('src/cells/infrastructure','infrastructure_cells','INFRA_LOCK_v2.1','infrastructure/ excluded — DB adapters high churn');
...
const bl=buildLock('src/cells/business','business_cells','BUSINESS_LOCK_v2.1','domain+application=business logic core. ports=public API.');
...
function countTs(dir){
  ...
  function walk(d){
    try{
      fs.readdirSync(d).forEach(f=>{
        const fp=path.join(d,f), s=fs.statSync(fp);
        if(s.isDirectory() && f!=='node_modules' && !f.startsWith('__tests__')) walk(fp);
        else if( (f.endsWith('.ts')||f.endsWith('.tsx')) && !f.endsWith('.spec.ts') && !f.endsWith('.test.ts')) n++;
      });
    }catch(e){}
  }
  walk(dir); return n;
...
function inferStatus(cellPath,existing){
  if(existing==='QUARANTINED'&&!FORCE) return 'QUARANTINED';
...
if(!DRY_RUN){
  reg.summary.last_synced=new Date().toISOString();reg.summary.sync_method='bang-goldmaster-v2.1';
...
NODEJS_EOF
fi  # kết thúc AUDIT_ONLY

# ================================================================
log_h "TASK 4: SEMANTIC AUDIT — 7 BLOCKS — GOLD MASTER v2.1"
# ================================================================

SEMANTIC_REPORT="goldmaster_semantic_$(date '+%Y%m%d_%H%M%S').md"
cat > "$SEMANTIC_REPORT" << MDEOF
# NATT-OS Semantic Audit — Gold Master v2.1
...
# 1a Wave sequence
P=$(find src/cells/business/pricing-cell -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ' || echo 0)
I=$(find src/cells/business/inventory-cell -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l | tr -d ' ' || echo 0)
...
if [ "$BK" -eq 0 ]; then
  ...
  grep -r "cells/kernel" src/cells/business --include="*.ts" 2>/dev/null | grep -E "import|from" | grep -v "//\|test" | head -3 | while read l; do log_data "$l"; done || true
fi
echo "- [1b] Business→Kernel: $BK" >> "$SEMANTIC_REPORT"

# 1c Cross-cell service coupling
CROSS=0
for cd in src/cells/business/*/; do
  cn=$(basename "$cd")
  for od in src/cells/business/*/; do
    on=$(basename "$od"); [ "$cn" = "$on" ] && continue
    h=$(grep -r "from.*${on}.*service\|require.*${on}.*service" "${cd}" --include="*.ts" 2>/dev/null \
        | grep -v "//\|test\|interface\|Port\|port" | wc -l | tr -d ' ' || echo 0)
    CROSS=$((CROSS+h))
  done
done
...
# 1d Audit-by-default
UCF=$(find src/cells -path "*/use-cases/*.ts" -o -path "*/use-case/*.ts" 2>/dev/null | grep -v node_modules | wc -l | tr -d ' ' || echo 0)
UCA=0; RATIO=0
if [ "$UCF" -gt 0 ]; then
  UCA=$(find src/cells -path "*/use-cases/*.ts" -o -path "*/use-case/*.ts" 2>/dev/null | grep -v node_modules | xargs grep -l "audit\|AuditPort\|AuditEvent" 2>/dev/null | wc -l | tr -d ' ' || echo 0)
  RATIO=$((UCA*100/UCF))
fi
...
# ─────────────────────────────────────────────────────────────
log_h2 "BLOCK 2: Dependency Direction"
...
[ "$KB" -eq 0 ] && { log_ok "2a Kernel→Business: 0 ✓"; B2_PASS=$((B2_PASS+1)); } \
                || { log_fail "2a CRITICAL Kernel→Business: $KB"; B2_FAIL=$((B2_FAIL+1)); grep -r "cells/business" src/cells/kernel --include="*.ts" 2>/dev/null | grep -E "import|from" | head -3 | while read l; do log_data "$l"; done || true; }
...
for cd in src/cells/business/*/; do
  ...
  v=$(grep -r "from.*infrastructure\|require.*infrastructure" "$d" --include="*.ts" 2>/dev/null | grep -v "//\|test" | wc -l | tr -d ' ' || echo 0)
  DI=$((DI+v))
done
...
for cd in src/cells/business/*/; do
  ...
  for od in src/cells/business/*/; do
    on=$(basename "$od"); [ "$cn" = "$on" ] && continue
    grep -qr "$on" "${cd}" --include="*.ts" 2>/dev/null && grep -qr "$cn" "${od}" --include="*.ts" 2>/dev/null && { log_fail "2d Circular: $cn ↔ $on"; CIRC=$((CIRC+1)); } || true
  done
done
...
HE=$(grep -r "\.emit\s*(\s*['\"]" src/cells --include="*.ts" 2>/dev/null | grep -v "//\|test\|spec" | wc -l | tr -d ' ' || echo 0)
...
EF=$(find src/cells -name "*EventEmitter*.ts" 2>/dev/null | grep -v node_modules | wc -l | tr -d ' ' || echo 0)
EA=0; [ "$EF" -gt 0 ] && EA=$(find src/cells -name "*EventEmitter*.ts" 2>/dev/null | grep -v node_modules | xargs grep -l "audit\|log\|record" 2>/dev/null | wc -l | tr -d ' ' || echo 0)
...
for cd in src/cells/business/*/; do
  ...
  s=$(grep -r "$STUB_PATTERNS" "$app" --include="*.ts" 2>/dev/null | grep -v "//\|test\|spec\|mock\|abstract\|constructor" | wc -l | tr -d ' ' || echo 0)
  AS=$((AS+s))
done
...
for cd in src/cells/business/*/; do
  ...
  s=$(grep -r "$STUB_PATTERNS" "$inf" --include="*.ts" 2>/dev/null | grep -v "test\|spec\|mock" | wc -l | tr -d ' ' || echo 0)
  IS=$((IS+s))
done
log_info "5c Infrastructure stubs: $IS (informational — adapters acceptable)"

SVC=$(grep -r "$STUB_PATTERNS" src/services --include="*.ts" 2>/dev/null | grep -v "test\|spec\|mock" | wc -l | tr -d ' ' || echo 0)
...
for cd in src/cells/business/*/; do
  ...
  for od in src/cells/business/*/; do
    on=$(basename "$od"); [ "$cn" = "$on" ] && continue
    h=$(grep -r "from.*cells/business/${on}\|from.*@.*${on}" "${cd}" --include="*.ts" 2>/dev/null | grep -v "//\|test\|shared-contracts" | wc -l | tr -d ' ' || echo 0)
    CE=$((CE+h))
  done
done
...
TI=$(grep -r "from.*['\"]@/types\|from.*['\"]../../types\|from.*['\"]../types" src/cells --include="*.ts" 2>/dev/null | grep -v "//\|node_modules" | wc -l | tr -d ' ' || echo 0)
...
# ─────────────────────────────────────────────────────────────
log_h2 "BLOCK 7: Governance Freeze"
...
for(const qc of qCells){
  for(const sd of ['src/cells/business','src/cells/infrastructure','src/cells/kernel','src/components','src/services']){
    ...
    try{
      const n=parseInt(execSync(`grep -r "${qc}" "${sd}" --include="*.ts" --include="*.tsx" 2>/dev/null | grep -E "import|from|require" | grep -v "//|test|spec" | wc -l`,{encoding:'utf8'}).trim())||0;
...
echo ""
echo -e "${BOLD}┌──────────────────────────────────────────────────────────────┐${NC}"
echo -e "${BOLD}│         SEMANTIC AUDIT SCORECARD v2.1                        │${NC}"
...
| 5. Anti-Stub | $B5_PASS | $B5_WARN | $B5_FAIL | $([ $B5_FAIL -eq 0 ] && echo '✅' || echo '❌') |
...
### v2.1 Improvements
- Thêm `|| echo 0` vào các pipeline grep để tránh script dừng.
- Xử lý TSC errors lấy dòng đầu.
- Lock loại trừ file test (`.spec.ts`, `.test.ts`, `__tests__/`).
- Alias detection lấy tất cả alias.
- Tùy chọn `--audit-only` chạy audit không ảnh hưởng lock/registry.
...
# Audit trail
AE="{\"timestamp\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"op\":\"GOLDMASTER_v2.1\",\"tsc_errors\":$TSC_ERRORS,\"score\":$PCT,\"grade\":\"$GRADE\",\"critical_fails\":$TF,\"dry_run\":$DRY_RUN,\"audit_only\":$AUDIT_ONLY}"
...
echo ""
echo -e "${BOLD}${GREEN}GOLD MASTER v2.1 COMPLETE — $(date '+%Y-%m-%d %H:%M:%S')${NC}"
echo ""
